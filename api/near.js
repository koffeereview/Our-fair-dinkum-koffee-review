...
export default async function handler(req, res) {
  try {
    ...
    if (!landmark || !LANDMARKS[landmark]) {
      var linkList = Object.keys(LANDMARKS).map(function(k) {
        return '<a href="/coffee-near/' + k + '" style="color:#E6C073;text-decoration:none;display:inline-block;padding:6px 12px;border:1px solid rgba(230,192,115,0.3);border-radius:20px;margin:4px">' + LANDMARKS[k].name + '</a>';
      }).join("");
      res.setHeader("Content-Type", "text/html; charset=utf-8");
      return res.status(landmark ? 404 : 200).send('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Coffee Near Me — Brisbane Landmarks | Koffee Review</title><meta name="description" content="Find the best coffee near popular Brisbane and Gold Coast landmarks. Honest reviews by Koffee Review."><link rel="canonical" href="https://koffeereview.com.au/coffee-near-me"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:sans-serif;background:#000;color:#fff;padding:60px 24px;text-align:center;max-width:800px;margin:0 auto}</style></head><body><h1 style="font-size:32px;margin-bottom:8px">Coffee Near Landmarks</h1><p style="color:rgba(255,255,255,0.5);margin-bottom:32px">Find the best-reviewed cafes near popular spots</p><div>' + linkList + '</div><p style="margin-top:40px"><a href="/" style="color:#E6C073">← Back to Koffee Review</a></p></body></html>');
    }
    ...
    var faqSchema = JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[
      {"@type":"Question","name":"Where is the best coffee near " + lm.name + "?","acceptedAnswer":{"@type":"Answer","text":"The highest-rated cafe near " + lm.name + " is " + nearby[0].name + " in " + nearby[0].suburb + " (" + nearby[0].score + "/10), about " + nearby[0]._dist.toFixed(1) + " km away."}},
    ...
    // FAQ section
    html += '<section style="margin-top:48px;padding-top:32px;border-top:1px solid rgba(255,255,255,0.08)"><h2 class="st">FREQUENTLY ASKED</h2><details><summary>Where is the best coffee near ' + lm.name + '?</summary><p>The highest-rated cafe near ' + lm.name + ' is ' + escapeHtml(nearby[0].name) + ' in ' + escapeHtml(nearby[0].suburb) + ' (' + nearby[0].score + '/10), about ' + nearby[0]._dist.toFixed(1) + ' km away.</p></details><details><summary>How many cafes are near ' + lm.name + '?</summary><p>We found ' + nearby.length + ' reviewed cafes within driving distance of ' + lm.name + ', ' + lm.city + '.</p></details><details><summary>How does Koffee Review rate cafes?</summary><p>We order one latte and one double espresso at every cafe. Same order, same size, every time. No sponsorships, no freebies.</p></details></section>';
