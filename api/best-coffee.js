...
function renderCityPage(citySlug, cafes) {
  ...
  const canonicalUrl = "https://koffeereview.com.au/best-coffee-"+citySlug;
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .top-card{padding:12px 6px}.top-sc{font-size:24px}.top-nm{font-size:11px}.stats{display:grid;grid-template-columns:1fr 1fr}.stat{border-right:none;border-bottom:1px solid rgba(255,255,255,0.04);padding:12px 8px;min-width:0}.stat:nth-child(odd){border-right:1px solid rgba(255,255,255,0.04)}.stat:nth-last-child(-n+2){border-bottom:none}}
      @media(max-width:480px){body{overflow-x:hidden}.c,.pg,.page{padding:0 14px!important;max-width:100vw}.stats{display:grid!important;grid-template-columns:1fr 1fr}.stat{border-right:none!important;border-bottom:1px solid rgba(255,255,255,0.04);padding:12px 8px;min-width:0!important}.stat:nth-child(odd){border-right:1px solid rgba(255,255,255,0.04)!important}.stat:nth-last-child(-n+2){border-bottom:none!important}nav,.nav{padding:12px 0!important}.nav-links{gap:10px}.nav-links a{font-size:10px!important;letter-spacing:1px}.nav-logo span,.nav-logo-text{font-size:14px!important;letter-spacing:2px}.nav-logo img{width:30px!important;height:30px!important}h1{font-size:clamp(26px,7vw,36px)!important}.hero-sub{font-size:12px!important}.gold-line{margin:10px 0!important}.cc{padding:12px 14px!important;gap:10px!important}.cc-bar{width:3px}.cc-sc{font-size:20px!important;min-width:36px!important;margin-left:4px!important}.cc-nm{font-size:13px!important}.cc-loc{font-size:10px!important}.cc-nt{font-size:11px!important;max-width:160px!important}.cc-vd{font-size:8px!important;padding:2px 7px!important;letter-spacing:1px!important}.filter-bar{gap:6px!important}select{font-size:11px!important;padding:7px 28px 7px 12px!important}.near-btn{font-size:11px!important;padding:7px 12px!important}.stat-n,.sp-n{font-size:22px!important}.stat-l,.sp-l{font-size:8px!important;letter-spacing:1.5px!important}.section-title,.sh-label{font-size:13px!important}.lm-btn{padding:12px!important;font-size:12px!important}.ft{padding:16px 0!important}.ft a{font-size:10px!important}img{max-width:100%!important;height:auto!important}}.top3{grid-template-columns:1fr 1fr 1fr!important;gap:6px}.top-card{padding:12px 6px!important}.top-sc{font-size:22px!important}.top-nm{font-size:11px!important}
  </style>
</head>
<body>
  <div class="c">
...
export default async function handler(req, res) {
  try {
    const citySlug = (req.query.city || "brisbane").toLowerCase().trim();
    const response = await fetch(SHEET_URL);
    const text = await response.text();
    const cafes = parseCSV(text);
    const html = renderCityPage(citySlug, cafes);
    if (!html) { res.status(404).send('<!DOCTYPE html><html><head><title>City Not Found</title><meta name="robots" content="noindex"><link rel="icon" href="/logo.webp"></head><body style="font-family:sans-serif;background:#0d0d0f;color:#fff;text-align:center;padding:60px"><h1 style="color:#E6C073">City not found</h1><p style="color:rgba(255,255,255,0.5)">We haven\'t reviewed cafés in this city yet.</p><a href="/" style="color:#E6C073">Browse All Reviews</a></body></html>'); return; }
