// BULLETPROOF SITEMAP — proper CSV parsing + XML escaping + cannot 500
...
function safeParse(text) {
  try {
    ...
    const headers = parseCSVLine(lines[0]).map(function(h) { return String(h || "").trim().toLowerCase(); });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error("Sitemap error:", error && error.message);
    return res.status(200).send(fallbackXML);
  }
}
