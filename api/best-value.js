...
function extractPrice(p){var m=(p||"").match(/\$?\s*(\d+(?:\.\d+)?)/);return m?parseFloat(m[1]):0;}
...
export default async function handler(req,res){
  try{
    ...
    cafes.push({name:n,suburb:(p[si]||"").trim(),city:city,score:sc,notes:(p[noi]||"").trim(),price:price,priceStr:(p[pi]||"").trim(),value:valueScore});}catch(e){}}
    ...
    var top=sorted[0];
    var avgPrice=(cafes.reduce(function(s,c){return s+c.price;},0)/total).toFixed(2);
    ...
    var desc="Every cafe in "+cityDisplay+" ranked by score per dollar. "+total+" cafes with price data. Best value: "+esc(top.name)+" ("+top.score.toFixed(1)+"/10 at $"+top.price.toFixed(2)+"). Find great coffee without overpaying.";
    ...
    var noscriptCards=sorted.slice(0,20).map(function(c,i){
      ...
      return'<a href="/review/'+slug+'" style="display:block;padding:10px;margin-bottom:4px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:8px;text-decoration:none;color:#fff;font-size:13px">'+(i+1)+'. '+esc(c.name)+' ('+esc(c.suburb)+') — <span style="color:'+col+'">'+c.score.toFixed(1)+'/10</span> at $'+c.price.toFixed(2)+'</a>';
    }).join("");

    var faqSchema=JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[
      {"@type":"Question","name":"Where is the best value coffee in "+cityDisplay+"?","acceptedAnswer":{"@type":"Answer","text":"Based on "+total+" cafes with price data, "+top.name+" in "+top.suburb+" offers the best value in "+cityDisplay+" with a score of "+top.score.toFixed(1)+"/10 at $"+top.price.toFixed(2)+". We calculate value by dividing the quality score by the latte price."}},
      {"@type":"Question","name":"How much does good coffee cost in "+cityDisplay+"?","acceptedAnswer":{"@type":"Answer","text":"The average latte price across "+total+" "+cityDisplay+" cafes is $"+avgPrice+". "+mustVisit+" cafes score 7.5+ (Must Visit). Good coffee does not always mean expensive coffee."}}
    ]});
    ...
    +'<div class="stats"><div class="stat"><div class="stat-n">'+total+'</div><div class="stat-l">WITH PRICE DATA</div></div><div class="stat"><div class="stat-n">$'+avgPrice+'</div><div class="stat-l">AVG PRICE</div></div><div class="stat"><div class="stat-n">'+top.score.toFixed(1)+'</div><div class="stat-l">BEST VALUE SCORE</div></div><div class="stat"><div class="stat-n">$'+top.price.toFixed(2)+'</div><div class="stat-l">AT PRICE</div></div></div>'
